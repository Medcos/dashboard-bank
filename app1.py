...
def update_client_predict(client_id, n_clicks):
    if client_id and n_clicks > 0:
        response = requests.get(f'http://127.0.0.1:5000/predict/{client_id}')
        if response.status_code == 200:
...
def update_local_interpretation(client_id, n_clicks):
    if n_clicks > 0 :
        # Ouvre le graphique
       return dcc.Location(href=f'http://127.0.0.1:5000/interpretation/local/{client_id}', id='inter-loc')   
...
def load_image_globale():
    response = requests.get('http://127.0.0.1:5000/interpretation/global')
    if response.status_code == 200:
...
def analysis_drift(n_clicks):
    if n_clicks > 0:
        ...
        return html.A('Ouvrir l\'analyse de drift dans un nouveau onglet', href='http://127.0.0.1:5000/drift', target="_blank")
    return dash.no_update
